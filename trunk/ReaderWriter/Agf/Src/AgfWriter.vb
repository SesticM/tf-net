...
''' <summary>
''' Reads features based on JTS model and creates their Autodesk Feature Data Objects (FDO) representation.
''' </summary>
''' <remarks></remarks>
Public Class AgfWriter
    Inherits GeometryWriter

    Private m_Factory As Autodesk.Gis.Geometry.AgfGeometryFactory
...
#Region " CTOR "
    ...
    ''' <summary>
    ''' Initializes a new instance of the <see cref="AgfWriter"/> class, using
    ''' user-supplied <see cref="Autodesk.Gis.Geometry.AgfGeometryFactory"/> for geometry processing. 
    ''' </summary>
    ''' <param name="factory">A <see cref="Autodesk.Gis.Geometry.AgfGeometryFactory"/>.</param>
    ''' <remarks></remarks>
    Sub New(ByVal factory As Autodesk.Gis.Geometry.AgfGeometryFactory)
        MyBase.New()
        m_Factory = factory
    End Sub
...
#Region " Factory "
    ...
    ''' <summary>
    ''' Returns current <see cref="Autodesk.Gis.Geometry.AgfGeometryFactory"/> used to build geometries.
    ''' <para>
    ''' It's actually reserved for possible future releases of <see cref="Autodesk.Gis.Geometry.AgfGeometryFactory"/>
    ...
    ''' </summary>
    ''' <value></value>
    ''' <returns>Current <see cref="Autodesk.Gis.Geometry.AgfGeometryFactory"/> instance.</returns>
    ''' <remarks>
    ''' If there's no <see cref="Autodesk.Gis.Geometry.AgfGeometryFactory"/> set within class constructor,
    ''' a <c>Default</c> factory will be automatically instantiated. Otherwise,
    ''' user-supplied <see cref="Autodesk.Gis.Geometry.AgfGeometryFactory"/> will be used during geometries
    ''' building process.
    ''' </remarks>
    Private ReadOnly Property Factory() As Autodesk.Gis.Geometry.AgfGeometryFactory
        Get
            If m_Factory Is Nothing Then
                m_Factory = New Autodesk.Gis.Geometry.AgfGeometryFactory
            End If
            Return m_Factory
        End Get
    End Property
...
#Region " WriteDirectPosition "
    ...
    ''' <summary>
    ''' Returns <see cref="Autodesk.Gis.Geometry.IDirectPosition"/> structure converted from <see cref="Coordinate"/> structure.
    ''' If <see cref="Coordinate"/> is two-dimensional, resulting <see cref="Autodesk.Gis.Geometry.IDirectPosition"/> is
    ...
    ''' <returns>A <see cref="Autodesk.Gis.Geometry.IDirectPosition"/> structure.</returns>
    ''' <remarks></remarks>
    Public Function WriteDirectPosition(ByVal coordinate As ICoordinate) As Autodesk.Gis.Geometry.IDirectPosition
    ...
    ''' If <see cref="Coordinate"/> is two-dimensional, resulting <see cref="Autodesk.Gis.Geometry.IDirectPosition"/>
    ...
    Public Function WriteDirectPosition(ByVal coordinate As ICoordinate, ByVal format As BinaryFormat) As Byte()
        If format = BinaryFormat.FGF Then
            Return Me.Factory.GetAgf(Me.WriteDirectPosition(coordinate))
        Else
...
#Region " WritePoint "
    ...
    ''' <summary>
    ''' Returns <see cref="Autodesk.Gis.Geometry.IPoint"/> geometry converted from <see cref="Point"/> geometry.
    ''' </summary>
    ''' <param name="point">A <see cref="Point"/> geometry.</param>
    ''' <returns>A <see cref="Autodesk.Gis.Geometry.IPoint"/> geometry.</returns>
    ''' <remarks></remarks>
    Public Function WritePoint(ByVal point As IPoint) As Autodesk.Gis.Geometry.IPoint
    ...
    Public Function WritePoint(ByVal point As IPoint, ByVal format As BinaryFormat) As Byte()
        If format = BinaryFormat.FGF Then
            Return Me.Factory.GetAgf(Me.WritePoint(point))
        Else
...
#Region " WriteLineString "
    ...
    ''' <summary>
    ''' Returns <see cref="Autodesk.Gis.Geometry.ILineString"/> geometry converted from <see cref="LineString"/> geometry.
    ''' </summary>
    ''' <param name="lineString">A <see cref="LineString"/> geometry.</param>
    ''' <returns>A <see cref="Autodesk.Gis.Geometry.ILineString"/> geometry.</returns>
    ''' <remarks></remarks>
    Public Function WriteLineString(ByVal lineString As ILineString) As Autodesk.Gis.Geometry.ILineString
    ...
    Public Function WriteLineString(ByVal lineString As ILineString, ByVal format As BinaryFormat) As Byte()
        If format = BinaryFormat.FGF Then
            Return Me.Factory.GetAgf(Me.WriteLineString(lineString))
        Else
...
#Region " WriteLinearRing "
    ...
    ''' <summary>
    ''' Returns <see cref="Autodesk.Gis.Geometry.ILinearRing"/> geometry converted from <see cref="LinearRing"/> geometry.
    ''' </summary>
    ''' <param name="linearRing">A <see cref="LinearRing"/> geometry.</param>
    ''' <returns>A <see cref="Autodesk.Gis.Geometry.ILinearRing"/> geometry.</returns>
    ''' <remarks></remarks>
    Public Function WriteLinearRing(ByVal linearRing As ILinearRing) As Autodesk.Gis.Geometry.ILinearRing
    ...
    Public Function WriteLinearRing(ByVal linearRing As ILinearRing, ByVal format As BinaryFormat) As Byte()
        If format = BinaryFormat.FGF Then
            Return Me.Factory.GetAgf(Me.WriteLinearRing(linearRing))
        Else
...
#Region " WritePolygon "
    ...
    ''' <summary>
    ''' Returns <see cref="Autodesk.Gis.Geometry.IPolygon"/> geometry converted from <see cref="Polygon"/> geometry.
    ''' </summary>
    ''' <param name="polygon">A <see cref="Polygon"/> geometry.</param>
    ''' <returns>A <see cref="Autodesk.Gis.Geometry.IPolygon"/> geometry.</returns>
    ''' <remarks></remarks>
    Public Function WritePolygon(ByVal polygon As IPolygon) As Autodesk.Gis.Geometry.IPolygon
        Dim extRing As Autodesk.Gis.Geometry.ILinearRing = Me.WriteLinearRing(polygon.Shell)

        Dim ringCol As New Autodesk.Gis.Geometry.LinearRingCollection
        For Each hole As LinearRing In polygon.Holes
            Dim intRing As Autodesk.Gis.Geometry.ILinearRing = Me.WriteLinearRing(hole)
            ringCol.Add(intRing)
        Next
    ...
    Public Function WritePolygon(ByVal polygon As IPolygon, ByVal format As BinaryFormat) As Byte()
        If format = BinaryFormat.FGF Then
            Return Me.Factory.GetAgf(Me.WritePolygon(polygon))
        Else
...
#Region " WriteGeometry "
    ...
    ''' <summary>
    ''' Returns <see cref="Autodesk.Gis.Geometry.IGeometry"/> converted from <see cref="Geometry"/>.
    ''' <para>
    ...
    ''' <returns>A <see cref="Autodesk.Gis.Geometry.IGeometry"/> object.</returns>
    ''' <remarks></remarks>
    Public Function WriteGeometry(ByVal geometry As IGeometry) As Autodesk.Gis.Geometry.IGeometry
    ...
    Public Function WriteGeometry(ByVal geometry As IGeometry, ByVal format As BinaryFormat) As Byte()
        If format = BinaryFormat.FGF Then
            Return Me.Factory.GetAgf(Me.WriteGeometry(geometry))
        Else
...
#Region " WriteMultiPoint "
    ...
    ''' <summary>
    ''' Returns <see cref="Autodesk.Gis.Geometry.IMultiPoint"/> geometry converted from <see cref="MultiPoint"/> geometry.
    ''' </summary>
    ''' <param name="multiPoint">A <see cref="MultiPoint"/> geometry.</param>
    ''' <returns>A <see cref="Autodesk.Gis.Geometry.IMultiPoint"/> geometry.</returns>
    ''' <remarks></remarks>
    Public Function WriteMultiPoint(ByVal multiPoint As IMultiPoint) As Autodesk.Gis.Geometry.IMultiPoint
    ...
    Public Function WriteMultiPoint(ByVal multiPoint As IMultiPoint, ByVal format As BinaryFormat) As Byte()
        If format = BinaryFormat.FGF Then
            Return Me.Factory.GetAgf(Me.WriteMultiPoint(multiPoint))
        Else
...
#Region " WriteMultiLineString "
    ...
    ''' <summary>
    ''' Returns <see cref="Autodesk.Gis.Geometry.IMultiLineString"/> geometry converted from <see cref="MultiLineString"/> geometry.
    ''' </summary>
    ''' <param name="multiLineString">A <see cref="MultiLineString"/> geometry.</param>
    ''' <returns>A <see cref="Autodesk.Gis.Geometry.IMultiLineString"/> geometry.</returns>
    ''' <remarks></remarks>
    Public Function WriteMultiLineString(ByVal multiLineString As IMultiLineString) As Autodesk.Gis.Geometry.IMultiLineString
    ...
    Public Function WriteMultiLineString(ByVal multiLineString As IMultiLineString, ByVal format As BinaryFormat) As Byte()
        If format = BinaryFormat.FGF Then
            Return Me.Factory.GetAgf(Me.WriteMultiLineString(multiLineString))
        Else
...
#Region " WriteMultiPolygon "
    ...
    ''' <summary>
    ''' Returns <see cref="Autodesk.Gis.Geometry.IMultiPolygon"/> geometry converted from <see cref="MultiPolygon"/> geometry.
    ''' </summary>
    ''' <param name="multiPolygon">A <see cref="MultiPolygon"/> geometry.</param>
    ''' <returns>A <see cref="Autodesk.Gis.Geometry.IMultiPolygon"/> geometry.</returns>
    ''' <remarks></remarks>
    Public Function WriteMultiPolygon(ByVal multiPolygon As IMultiPolygon) As Autodesk.Gis.Geometry.IMultiPolygon
    ...
    Public Function WriteMultiPolygon(ByVal multiPolygon As IMultiPolygon, ByVal format As BinaryFormat) As Byte()
        If format = BinaryFormat.FGF Then
            Return Me.Factory.GetAgf(Me.WriteMultiPolygon(multiPolygon))
        Else
...
#Region " WriteMultiGeometry "
    ...
    ''' <summary>
    ''' Returns <see cref="Autodesk.Gis.Geometry.IMultiGeometry"/> geometry converted from <see cref="IMultiSurface"/> geometry.
    ''' </summary>
    ''' <param name="multiSurface">A <see cref="IMultiSurface"/> geometry.</param>
    ''' <returns>A <see cref="Autodesk.Gis.Geometry.IMultiGeometry"/> geometry.</returns>
    ''' <remarks></remarks>
    Public Function WriteMultiGeometry(ByVal multiSurface As IMultiSurface) As Autodesk.Gis.Geometry.IMultiGeometry
    ...
    Public Function WriteMultiGeometry(ByVal multiSurface As IMultiSurface, ByVal format As BinaryFormat) As Byte()
        If format = BinaryFormat.FGF Then
            Return Me.Factory.GetAgf(Me.WriteMultiGeometry(multiSurface))
        Else
...
#Region " WritePointCollection "
    ...
    ''' <summary>
    ''' Returns <see cref="Autodesk.Gis.Geometry.PointCollection"/> converted from array of <see cref="Point"/>s.
    ''' </summary>
    ''' <param name="points">An array of <see cref="Point"/>s.</param>
    ''' <returns>A <see cref="Autodesk.Gis.Geometry.PointCollection"/> collection.</returns>
    ''' <remarks></remarks>
    Public Function WritePointCollection(ByVal points() As IPoint) As Autodesk.Gis.Geometry.PointCollection
        Dim collection As New Autodesk.Gis.Geometry.PointCollection
...
#Region " WriteLineStringCollection "
    ...
    ''' <summary>
    ''' Returns <see cref="Autodesk.Gis.Geometry.LineStringCollection"/> converted from array of <see cref="LineString"/>s.
    ''' </summary>
    ''' <param name="lineStrings">An array of <see cref="LineString"/>s.</param>
    ''' <returns>A <see cref="Autodesk.Gis.Geometry.LineStringCollection"/> collection.</returns>
    ''' <remarks></remarks>
    Public Function WriteLineStringCollection(ByVal lineStrings() As ILineString) As Autodesk.Gis.Geometry.LineStringCollection
        Dim collection As New Autodesk.Gis.Geometry.LineStringCollection
...
#Region " WriteLinearRingCollection "
    ...
    ''' <summary>
    ''' Returns <see cref="Autodesk.Gis.Geometry.LinearRingCollection"/> converted from array of <see cref="LinearRing"/>s.
    ''' </summary>
    ''' <param name="linearRings">An array of <see cref="LinearRing"/>s.</param>
    ''' <returns>A <see cref="Autodesk.Gis.Geometry.LinearRingCollection"/> collection.</returns>
    ''' <remarks></remarks>
    Public Function WriteLinearRingCollection(ByVal linearRings() As ILinearRing) As Autodesk.Gis.Geometry.LinearRingCollection
        Dim collection As New Autodesk.Gis.Geometry.LinearRingCollection
...
#Region " WritePolygonCollection "
    ...
    ''' <summary>
    ''' Returns <see cref="Autodesk.Gis.Geometry.PolygonCollection"/> converted from array of <see cref="Polygon"/>s.
    ''' </summary>
    ''' <param name="polygons">An array of <see cref="Polygon"/>s.</param>
    ''' <returns>A <see cref="Autodesk.Gis.Geometry.PolygonCollection"/> collection.</returns>
    ''' <remarks></remarks>
    Public Function WritePolygonCollection(ByVal polygons() As IPolygon) As Autodesk.Gis.Geometry.PolygonCollection
        Dim collection As New Autodesk.Gis.Geometry.PolygonCollection
...
#Region " WriteGeometryCollection "
    ...
    ''' <summary>
    ''' Returns <see cref="Autodesk.Gis.Geometry.GeometryCollection"/> converted from <see cref="GeometryCollection"/>.
    ''' </summary>
    ''' <param name="geometries">A <see cref="GeometryCollection"/> collection.</param>
    ''' <returns>A <see cref="Autodesk.Gis.Geometry.GeometryCollection"/> collection.</returns>
    ''' <remarks></remarks>
    Public Function WriteGeometryCollection(ByVal geometries As IGeometryCollection) As Autodesk.Gis.Geometry.GeometryCollection
        Dim collection As New Autodesk.Gis.Geometry.GeometryCollection
